				...
				?>
					<tr>
						<?php
							...
							?>
							<?php
							$connection = mysqli_connect("localhost", "id16084204_electre1", "}95G!e!)e08yutqh", "id16084204_electre") or die("Error " . mysqli_error($connection));
							$myvar = array("rangking"=>$i+1);
							$myvar2 = array("list"=>$list_alt[$i]);
							$myvar3 = array("nama"=>$list_nama[$i]);
							$myvar4 = array("jumlah"=>$mat_hasil_baru[$i]);
							$myvar5 = array("status"=>$status);
							$myvar6 = array("tahun"=>$tahun);
							
							
							
							$newvar= implode(" , ",$myvar);
							$newvar2= implode(" , ",$myvar2);
							$newvar3= implode(" , ",$myvar3);
							$newvar4= implode(" , ",$myvar4);
							$newvar5= implode(" , ",$myvar5);
							$newvar6= implode(" , ",$myvar6);
							
							
							$query = mysqli_query($connection, "SELECT rangking FROM hasil WHERE rangking = '$newvar'"); 
							
							if ($query->num_rows > 0) {
						echo "<script>alert('Perangkingan Telah Dilakukan Silahkan Download Dan Hapus data');</script>";
									}
									else {
										mysqli_query($connection, "INSERT INTO hasil(rangking,list,nama,jumlah,status,tahun) VALUES ('".$newvar."','".$newvar2."','".$newvar3."','".$newvar4."','".$newvar5."','".$newvar6."')");
										}
							}
